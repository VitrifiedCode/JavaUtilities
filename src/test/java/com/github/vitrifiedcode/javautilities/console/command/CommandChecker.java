...
public class CommandChecker
{
    ...
    public static void executeCommand(String s)
    {
        if(s.startsWith("/"))
        {
            String[] split = StringUtil.SPACE_PATTERN.split(s);
            String x = split[0];
            if(COMMANDS.containsKey(x))
            {
                if(!COMMANDS.get(x).execute(Arrays.copyOfRange(split, 1, split.length)).successful)
                {
                    System.out.println(COMMANDS.get(x).getUsage());
                }
            }
        }
    }
